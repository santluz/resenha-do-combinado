...
// Rodapé com Instagram dinâmico vindo do Firebase
    ...
    <footer className="bg-[#080808] border-t border-[#1C1C1C] py-12 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-10">
          {/* Logo */}
          <div>
            <h2 className="text-white text-3xl font-black uppercase leading-none"
              style={{ fontFamily: "'Bebas Neue', 'Impact', sans-serif" }}>
              Resenha do<span className="text-[#1A7A3A]"> Combinado</span>
            </h2>
            <p className="text-[#444] text-xs mt-1 uppercase tracking-widest">Futebol amador com alma</p>
          </div>
          ...
          <nav aria-label="Navegação do rodapé">
            <ul className="flex flex-wrap justify-center gap-x-6 gap-y-2">
              {[
                ...
                <li key={link.label}>
                  <a href={link.href}
                    className="text-[#555] hover:text-[#F5C518] text-xs uppercase tracking-widest transition-colors duration-200">
                    {link.label}
                  </a>
                </li>
              ))}
        ...
        <div className="h-px bg-[#1C1C1C] mb-8" />

        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-[#333] uppercase tracking-widest">
          <p>© {currentYear} Resenha do Combinado. Todos os direitos reservados.</p>
          <p>
            Produção:{" "}
            <a href="https://github.com/santluz" target="_blank" rel="noopener noreferrer"
              className="text-[#444] hover:text-[#1A7A3A] transition-colors">
              Edson Santana
            </a>
          </p>
        </div>
