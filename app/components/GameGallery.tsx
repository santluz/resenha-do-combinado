...
// Galeria com foto destaque + grade de até 6 fotos + lightbox
    ...
    <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-4"
      ...
      <div className="relative w-full max-w-4xl max-h-[85vh] aspect-video"
        ...
        <Image src={photos[current].src} alt={photos[current].alt} fill
          className="object-contain" sizes="100vw" priority unoptimized />
      </div>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-center">
        <p className="text-white/70 text-sm">{photos[current].alt}</p>
        <p className="text-[#555] text-xs mt-1">{current + 1} / {photos.length}</p>
      </div>
    </div>
  );
}
...
export default function GameGallery({ photos, featuredPhoto }: Props) {
  ...
  const [showFeaturedLightbox, setShowFeaturedLightbox] = useState(false);

  if (!featuredPhoto && photos.length === 0) return null;
  ...
  return (
    <section id="galeria" className="bg-[#111] py-20 px-6">
        ...
        {featuredPhoto && (
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-3">
              <span className="bg-[#F5C518] text-black text-xs font-black uppercase tracking-widest px-3 py-1 rounded">
                ⭐ Destaque do Dia
              </span>
            </div>
            <button
              onClick={() => setShowFeaturedLightbox(true)}
              className="relative w-full aspect-video rounded-xl overflow-hidden cursor-zoom-in group"
              aria-label="Ver foto destaque"
            >
              <Image
              ...
              {featuredPhoto.alt && (
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
                  <p className="text-white text-sm font-semibold">{featuredPhoto.alt}</p>
                </div>
              )}
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {photos.map((photo, index) => (
              <button
                ...
                className="group relative aspect-square overflow-hidden rounded-lg bg-[#1C1C1C] cursor-zoom-in focus:outline-none focus:ring-2 focus:ring-red-600"
      ...
      {/* Lightbox foto destaque */}
      {showFeaturedLightbox && featuredPhoto && (
        <Lightbox photos={[featuredPhoto]} index={0} onClose={() => setShowFeaturedLightbox(false)} />
      )}
    </section>
  );
}
