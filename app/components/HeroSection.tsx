      ...
      <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
        {logoUrl && (
          <div className="flex justify-center mb-6">
            <Image src={logoUrl} alt="Logo do grupo" width={100} height={100} className="rounded-full border-4 border-red-600 shadow-2xl object-cover" unoptimized />
          </div>
        )}
        ...
        <h1 className="text-[clamp(3rem,12vw,8rem)] font-black uppercase leading-none text-white" style={{ fontFamily: "'Bebas Neue', Impact, sans-serif" }}>
          Resenha do<br /><span className="text-red-600">Combinado</span>
        </h1>
        ...
        <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
          <button onClick={() => document.getElementById("ultima-resenha")?.scrollIntoView({ behavior: "smooth" })}
            className="inline-flex items-center gap-3 bg-red-600 hover:bg-red-700 text-white font-bold uppercase tracking-widest text-sm px-8 py-4 rounded transition-all">
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
            Assistir Última Entrevista
          </button>
          <a href="#entrevistas" className="inline-flex items-center border border-[#333] hover:border-[#F5C518] text-[#A0A0A0] hover:text-[#F5C518] font-semibold uppercase tracking-widest text-sm px-8 py-4 rounded transition-all">
            Ver Entrevistas
          </a>
        </div>
      </div>
