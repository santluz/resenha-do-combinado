...
// Suporta entrevistas com vídeo YouTube ou vídeo próprio
...
function InterviewCard({ interview }: { interview: Interview }) {
  ...
  // URL de destino: vídeo próprio abre numa nova aba, YouTube também
  const videoHref = interview.videoUrl
    ? interview.videoUrl
    : `https://youtube.com/watch?v=${interview.youtubeId}`;

  // Thumbnail: própria, ou gerada pelo YouTube, ou placeholder
      ...
      <div className="relative aspect-video overflow-hidden bg-[#0D0D0D]">
        {thumbSrc ? (
          <Image src={thumbSrc} alt={`Entrevista com ${interview.name}`} fill
            ...
            unoptimized={thumbSrc.includes("youtube.com") || thumbSrc.includes("firebasestorage")}
          />
        ) : (
