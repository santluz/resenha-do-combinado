  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${scrolled ? "bg-[#0D0D0D]/95 backdrop-blur-md border-b border-[#1C1C1C] shadow-lg" : "bg-transparent"}`}>
      <nav className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <a href={`https://instagram.com/${instagram}`} target="_blank" rel="noopener noreferrer" className="hidden md:inline-flex items-center gap-2 text-[#888] hover:text-[#F5C518] text-xs font-bold uppercase tracking-widest transition-colors">Instagram</a>
      ...
      {menuOpen && (
        <div className="md:hidden bg-[#0D0D0D]/98 backdrop-blur-md border-b border-[#1C1C1C]">
          <ul className="max-w-6xl mx-auto px-6 py-4 flex flex-col gap-3">
            ...
            <li><a href={`https://instagram.com/${instagram}`} target="_blank" rel="noopener noreferrer" className="text-[#888] hover:text-[#F5C518] text-sm font-semibold uppercase block py-2 transition-colors">Instagram @{instagram}</a></li>
          </ul>
        </div>
      )}
    </header>
